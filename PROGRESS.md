...
- **Active Phase:** 9
- **Active Step:** 9.1
- **Last Session Summary (2026-06-17, user-reported bug sweep):** Worked a 7-bug list from live Tauri play. Status of each (commits on `main`):
  ...
  - **#18 trainer lock not working + trainer approach not mirrored (DIAGNOSED, NOT shipped — deferred to a session with a working chaos harness + Rust/TS builds + live verify).** Two parts. (a) *Lock:* `MP_PKT_TRAINER_BUSY (0x15)`/`TRAINER_FREE (0x19)` are one-shot packets with NO beacon recovery; recv/send logic is correct in the lossless case, so the real-relay failure is dropped BUSY (under-lock) or dropped FREE (permanent over-lock). Rule-4 sanctioned fix: carry busy-state on `MP_PKT_STATE_BEACON`. The bridge/relay treat the beacon as opaque `{type:"raw",bytes:hex}` keyed only on size (9), so the *lowest-risk* implementation overloads the 3 idle battle-turn bytes (turnSeq/moveSlot/target, zero outside a coop battle) for localId/mapGroup/mapNum + a flag bit — NO packet-size change → zero framing-regression risk and zero Rust/TS edits. NOT shipped because its convergence under loss/reorder is exactly what `set_link_chaos` must prove, and the MCP harness is wedged this session; a bare timeout auto-clear is the rule-4-forbidden band-aid. (b) *Approach mirroring:* genuinely new — needs the spotting trainer's NPC coords/facing sent to the partner (new packet, all 5 layers) so the non-battling player sees the trainer turn/approach. Deferred to a session with a live harness.
...
- **Next Action:** (1) **User Tauri re-test** of the 2026-06-17 fixes — #15 (join mid-Oak-escort: ghost should now walk, not stick at door), #16/#17 (rival win → Pokédex scene fires, no re-trigger, no false mom-heal), #19 (double-battle no longer desyncs on enemy target), #20 (trainer mons use level moves), #21 (follower ghost: does it now *vanish* on the bad-gfx case, or does a tree still appear → tile-slot reuse?). (2) **#18 once the MCP harness is restarted:** implement the beacon-overload lock recovery (3 idle battle-turn bytes → busy localId/mapGroup/mapNum + flag bit, no size change) and verify convergence under `set_link_chaos(drop=0.3, seed=1)` role-swapped; then design the trainer-approach mirror packet (all 5 layers). (3) Still open from before: **Phase 6** (Tauri end-to-end) and **Phase 7** (PartyKit live deploy verify at `wss://pokefirered-coop.thereuben.partykit.dev`).
- **⚠️ Harness note:** the MCP gamestate server is wedged (`start_emulator` never signals ready; both instances DEAD). It must be restarted OUTSIDE Claude Code before any live/chaos verification — it cannot be restarted from within a session.
