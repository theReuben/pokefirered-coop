...
**RNG lockstep (implemented 2026-06-12):** Both ROMs must produce identical RNG outputs for battle-logic rolls. The original plan ("send `gBattleRngSeed` in `MP_PKT_PARTY_SYNC`") was insufficient as stated: the expansion's battle-logic rolls (`RandomUniform(RNG_*, …)`) draw from the shared `gRngValue` stream, which is also advanced by per-frame visual/animation draws — two free-running instances diverge immediately even with the same seed. The implemented mechanism: `src/multiplayer.c` defines strong overrides of the weak `RandomUniform`/`RandomUniformExcept`/`RandomWeightedArray`/`RandomElementArray` symbols from `src/random.c` (guarded `#if !TESTING` — `test/test_runner.c` installs its own strong versions in TESTING builds). During `BATTLE_TYPE_COOP` battles they draw from a dedicated xorshift32 stream (`gMultiplayerState.coopRngState`), otherwise they fall through to the `*Default` implementations. Seeding: the host mints a seed per battle in `CB2_CoopPartySelected` and ships it in the 4 trailing bytes of `MP_PKT_PARTY_SYNC`; `Multiplayer_SetupCoopBattle` copies the adopted seed into the stream state on both sides. If neither side has a role, both seeds are 0 and the stream's zero-remap constant keeps the sides in lockstep anyway. Known limitation: untagged `Random()`/`Random32()` calls inside battle logic (if any remain upstream) still draw from the shared stream and can diverge; speed-tie resolution interprets mirrored battler indices. (Amended 2026-07-20 — seed lockstep alone is insufficient for opponent AI: the doubles AI evaluates candidate targets in battler order and consumes lockstep draws *inside* each (attacker, target) evaluation with a branch-dependent draw count, so on the mirrored sims (local player = battler 0 on each) the same draws fed opposite physical matchups and the stream position itself diverged — after which every roll, including canonicalized target picks, differed. `ChooseMoveOrAction_Doubles` now iterates targets in role-canonical order via `Multiplayer_CoopAiEvalBattler` (host 0,1,2,3; guest 2,1,0,3) for coop opponent-side AI, which also makes the score-tie array physically aligned for any tie composition and supersedes the former `{0,2}`-only tie remap. Untagged `Random()` in reachable AI scoring paths (`AI_RandLessThan`, `ShouldTryOHKO`, `ProtectChecks`, protect double-use, risky all-stats-up) was converted to tagged lockstep draws; the safari-flee and tera-predict sites remain untagged, unreachable in coop.) (Amended 2026-07-21 — the canonical target/eval order is keyed on `gMultiplayerState.role`, which lives in the session tier (IWRAM) and is zeroed by `Multiplayer_Init` on every continue/new-game/savestate-reload. The Tauri bridge delivered the relay's role assignment exactly once via a `ROLE_DELIVERED` one-shot latch and never re-asserted it; any re-init *after* that delivery left `role == MP_ROLE_NONE`, and `Multiplayer_CanonicalPlayerTarget`/`Multiplayer_CoopAiEvalBattler` fall through to the host mapping when role is NONE — so **both** mirrored sims picked the same battler index (0), which is a different physical mon on each (host b0 vs guest b0), reintroducing the exact target desync this section fixes. This is the "persistent fact cached in IWRAM must be recoverable from a durable/heartbeat source" contract (ENGINEERING_DISCIPLINE rule 3/4): role now rides the transport heartbeat rather than a one-shot. `serial_bridge.rs` and `tools/mcp_gamestate/server.py` re-assert the last role byte every relay cycle whenever it disagrees with the connected assignment, exactly mirroring the existing `connState` heartbeat, so a post-init wipe self-heals within one cycle. Verified live 2026-07-21 in RB1 with correct roles held throughout (host=1/guest=2): rival Squirtle's `gAiBattleData->chosenTarget[1]` read 0 on host (battler 0 = Bulbasaur) and 2 on guest (battler 2 = Bulbasaur) — the same physical mon via role-mirrored indices — for a non-damaging Tail Whip, confirming target convergence independent of HP. Native guards: `TestCoopCanonicalTargetRoleConsistent` + `TestCoopAiEvalBattlerOrder` in `test/native/test_packets.c`.)
